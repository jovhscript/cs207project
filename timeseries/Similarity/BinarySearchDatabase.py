...
class BinaryTree(object):
    ...
    def get(self, key):
        ...
        while node is not None:
            if key < node.key:
                #print(node.key,"Larger than", key)
                node = self._follow(node.left_ref)
            elif key > node.key:
                #print(node.key, "Smaller than", key)
    ...
    def in_order_traversal(self):
        ...
        while node is not None or len(stack)>0:
            if node is not None:
                ...
                node = stack.pop()
                solution.append((node.key,self._follow(node.value_ref)))
                node = self._follow(node.right_ref)
        return solution    
...
class Storage(object):
    ...
    def _ensure_superblock(self):
        ...
        #if we are in the middle of superblock, pad file to get to next superblock
...
class DBDB(object):
    ...
    def __init__(self, f):
        self._storage = Storage(f) #makes a storage object
        self._tree = BinaryTree(self._storage) #makes a binaryTree object
